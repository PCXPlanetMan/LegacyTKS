...
namespace AssetBundles
{
    ...
    public class AssetBundleManager : MonoBehaviour
    {
        ...
        static public AssetBundleLoadAssetOperation LoadAssetSync(string assetBundleName, string assetName, System.Type type)
        {
            Log(LogType.Info, "Loading " + assetName + " from " + assetBundleName + " bundle");
            ...
            {
                assetBundleName = RemapVariantName(assetBundleName);
                LoadAssetBundleSyncOnMobile(assetBundleName);
                operation = new AssetBundleLoadAssetOperationSync(assetBundleName, assetName, type);
            }

            return operation;
        }

        static protected void LoadAssetBundleSyncOnMobile(string assetBundleName)
        {
            if (m_AssetBundleManifest == null)
            {
                Log(LogType.Error, "Please initialize AssetBundleManifest by calling AssetBundleManager.Initialize()");
            ...
            // Check if the assetBundle has already been processed.
            bool isAlreadyProcessed = LoadAssetBundleInternalSyncOnMobile(assetBundleName);

            // Load dependencies.
            if (!isAlreadyProcessed)
                LoadDependenciesSyncOnMobile(assetBundleName);
        }

        static protected bool LoadAssetBundleInternalSyncOnMobile(string assetBundleName)
        {
            // Already loaded.
            LoadedAssetBundle bundle = null;
            m_LoadedAssetBundles.TryGetValue(assetBundleName, out bundle);
            ...
            else
            {
                ...
                {
                    m_DownloadingErrors.Add(assetBundleName, "Load AssetBundle from File failed.");
                }
                else
                {
                    var assetBundle = new LoadedAssetBundle(myLoadedAssetBundle);
        ...
        // Where we get all the dependencies and load them all.
        static protected void LoadDependenciesSyncOnMobile(string assetBundleName)
        {
            if (m_AssetBundleManifest == null)
            {
                Log(LogType.Error, "Please initialize AssetBundleManifest by calling AssetBundleManager.Initialize()");
            ...
            m_Dependencies.Add(assetBundleName, dependencies);
            for (int i = 0; i < dependencies.Length; i++)
                LoadAssetBundleInternalSyncOnMobile(dependencies[i]);
        }
